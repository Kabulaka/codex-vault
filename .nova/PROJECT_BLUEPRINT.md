...
| 持久化 | Rusqlite 管理自有偏好和操作记录，并在 app-server 无法可靠给出置顶状态时只读查询 Codex 状态库的 `threads(id, is_pinned)` | [本地数据契约](architecture/data/local-session-data.md) |
...
| `src/adapters/codex/` | Codex app-server 生命周期、协议映射、能力探测和置顶状态只读兼容解析 | 优先采用 app-server 明确字段；兼容回退只以只读方式查询 Codex 状态库的会话 ID 与置顶位，不把协议或存储细节泄漏到领域层 |
...
| Codex 接入 | 通过本地 app-server 发现会话、探测能力并请求状态变更；必要时只读补齐置顶状态 | 向会话整理模块提供类型化快照；除置顶兼容读取外不访问 Codex 内部存储，所有状态变更仍只走 app-server |
...
| 契约 | 适用范围 | 验证 |
|------|----------|------|
| C-01 | 所有模块：Codex 所有权 | 会话及归档状态只由 app-server 改变；Codex 状态库仅允许只读查询 `threads(id, is_pinned)`，测试拒绝任何内部数据库和会话文件写路径 |
| C-02 | Codex 接入、会话整理、终端交互：能力门禁 | 必需稳定方法、实验树关系或置顶状态来源不完整时，写用例只能返回只读原因；app-server 置顶字段缺失或筛选失真时允许只读状态库兼容回退 |
...
| C-06 | 本地记录、Codex 接入、终端交互：最小数据 | 自有存储只含偏好、时间、动作、会话 ID 和结果；置顶回退只读取 ID 与布尔位；不保存标题快照、正文、凭据或 Codex 配置 |
...
| 边界 | 内容 |
|------|------|
| 本期必须实现 | Rust 单文件双语 TUI、本地 app-server 能力探测、元数据分页与筛选、置顶状态只读兼容解析、完整树预览、归档与恢复、受控删除、SQLite 偏好和操作记录、四目标 GitHub Releases |
| 明确不做 | Windows、远程 app-server、WebSocket、常驻服务、消息正文读取、Codex 内部数据库写入、除 `threads(id, is_pinned)` 外的内部数据库读取、会话文件访问、日志缓存清理、自动更新、crates.io 发布、macOS 签名与公证 |
| 后续候选 | Windows、包管理器与 crates.io、远程会话、签名公证、脚本化入口，以及独立的磁盘空间清理能力 |
| AI 可自行决定 | 不改变领域不变量和外部行为的内部函数拆分、终端配色、局部组件、查询实现与测试组织 |
| 必须再次确认 | 修改需求语义、增加外部服务或网络监听、读取消息正文、用内部存储执行状态变更、扩大 Codex 状态库只读字段或自有持久化字段、改变操作门禁、增加平台或发布渠道、引入签名凭据和显著运行依赖 |

## 6. 交付工作项

| 编号 | 状态 | 优先级 | 来源 | 功能 | 设计依据 | 前置依赖 | 完成定义 | 需求引用 |
|------|------|--------|------|------|----------|----------|----------|----------|
| FEAT-01a08e82-2ef2-761b-abb6-ca0feedfc1f4 | 待Review | P0 | 用户提出 | 交付 Codex Vault 本地会话整理 MVP | [本地会话整理 MVP](design/2026-09-11_本地会话整理MVP.md#wp-01-codex-vault-mvp) | 无 | 双语分阶段 TUI 可直观筛选并批量选择、预览、归档、恢复和受控删除完整交互会话树，失败后能据实核对与重试，发布矩阵和最低验收通过 | [REQ-01a08e5a-fe60-749a-8bc9-5b1b292e02d6@v1](requirements/REQ-01a08e5a-fe60-749a-8bc9-5b1b292e02d6_本地会话整理.md) |
...
| 基础设施层 | Codex 协议、置顶状态只读兼容查询与自有 SQLite 的端口实现 | `src/adapters/codex/`、`src/adapters/storage/` | 领域端口及各自专用库 |
...
| 适用范围 | 不变量 | 验证 |
|----------|--------|------|
| Codex 会话 | 会话发现、关系和状态变更通过 app-server；置顶状态优先取 app-server，缺失或筛选失真时仅以只读状态库补齐；任一来源或 ID 覆盖不完整时保持只读 | 协议与状态库 Mock 覆盖字段可靠、筛选失真、缺库、缺列、非法值、覆盖不全、活动节点和部分归档 |
...
| app-server 置顶字段缺失或筛选失真 | Codex 接入 | 尝试以只读模式查询当前 Codex 状态库的 `threads(id, is_pinned)`；无法完整覆盖本次扫描 ID 时保留展示并禁用写操作 |
