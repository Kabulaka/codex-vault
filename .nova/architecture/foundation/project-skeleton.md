...
|------|------|----------|--------|------|------|------|------|
| Rust 2024 | Ratatui、Crossterm、Tokio、Serde | 单个原生 TUI 进程拥有一个本地 `codex app-server` 标准输入输出子进程 | Rusqlite 保存自有偏好和操作记录，并只读查询 Codex 状态库的会话 ID 与置顶位 | 不使用 | 不使用消息系统；仅处理当前子进程 JSONL | 不单独鉴权，继承当前操作系统用户权限 | MIT；GitHub Releases 发布 Linux x86_64/ARM64 与 macOS Intel/Apple Silicon 二进制和 SHA-256；macOS 不签名、不公证 |
...
| `src/adapters/codex/` | app-server 传输、能力探测、协议映射和置顶状态只读兼容查询 | 领域端口、Tokio、Serde、Rusqlite 只读连接、最小文件系统定位 | Ratatui、Codex 数据库写入、除 `threads(id, is_pinned)` 外的查询、会话正文和领域策略判断 |
